...
dim = 2
delta_t = 0.0625
A = np.sqrt(delta_t) * np.tril(np.ones(dim))
b = np.random.uniform(-1, 1, size=dim)
sigma = 0.2
S_0 = 100
T = 1
r = 0.1
...
def test_func(A, x):
    ...
    S = S_0 * np.exp(
            (r - 0.5 * sigma**2) * T
            + sigma * np.sqrt(T).dot(stats.norm.ppf(x))
        )
    return np.maximum(0, S - K)


f = lambda x: test_func(A, x)
...
def plotFunction(opEval, surpluses, X):
    ...
    plt.savefig('/home/michael/plots/function-adaptive.png')
...
print(X.shape)
fX = pysgpp.DataVector(np.apply_along_axis(f, axis=1, arr=X))
